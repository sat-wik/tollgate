...
- *2026-06-03* — M2 Anthropic token approximation: o200k_base (tiktoken) is used as a proxy for the text, plus a `5 + 2×messages` framing overhead. Calibrated against real Haiku 4.5 `usage.input_tokens` (single-message English prompts, 2–1643 chars); every point landed within ±7%. Documented band: ±10% for English text-only; widened (and always marked "approx") for multimodal/heavy tool-use. OpenAI uses tiktoken with the documented chat formula (exact for text).
- *2026-06-03* — Pricing table citations live in per-entry `source`/`asOf` fields rather than comments (JSON has none); `Pricing.price()` / `priceRequest()` return `null` for an unknown model — surfaced as "unknown", never a guessed number.
- *2026-06-03* — M2 pre-flight estimate is emitted as additive `x-tollgate-*` response headers (body untouched). Persisted cost is computed from the provider's actual usage when present, falling back to the local input estimate, so every row carries a best-effort cost.
- *2026-06-03* — M3 lint rules are pure functions of the request; the deterministic token counter is injected via `LintContext.countText` (a cached o200k singleton) so rules stay free of I/O, clock, and global state. The engine isolates each rule in try/catch so a buggy rule can never break a request.
- *2026-06-03* — M3 caching opportunities are represented as a `cache-opportunity` Finding and stored in the same `findings` table, unifying persistence and the (future) dashboard. The cache detector is per-route (its own rolling window) and compares the normalized system + first-user-message prefix, not raw bodies.
- *2026-06-03* — Document (PDF) token estimates use page count when known (~1.5k/page) else bytes; images are bounded to a high-detail ballpark. All multimodal estimates are coarse and always feed an "approx" result. Refinement deferred.
- *2026-06-03* — M4 dashboard is a self-contained, dependency-free HTML page (no external scripts/styles/fonts) served at `/_tollgate`, satisfying the offline invariant. It is a thin read-only viewer over JSON read-model endpoints; raw prompt content is never exposed (only stored metadata, costs, findings). The page supports an optional embedded dataset so a static snapshot can be exported without a server.
- *2026-06-03* — Added a `request_type` column for dashboard attribution via an additive migration (PRAGMA table_info + ALTER TABLE), so stores created by the earlier schema upgrade in place. Classification ("multimodal"/"long-context"/"short-prompt") is a coarse heuristic computed at insert time.
- *2026-06-03* — M5 entrypoint dispatches subcommands; `tollgate init` scaffolds `~/.tollgate/config.toml` from the bundled default and is idempotent (never clobbers an existing config).
- *2026-06-03* — M5 build emits to `dist/` via `tsconfig.build.json` (excludes tests/scripts), and a `copy-assets.mjs` post-step copies fs-read runtime assets (`config/default.toml`, `pricing/table.json`, `store/schema.sql`) into `dist/` at the same `__dirname`-relative paths, so the built `tollgate` binary resolves them identically to dev (`tsx`). `bin` points at `dist/src/index.js` (shebang preserved by tsc).
- *2026-06-03* — Per maintainer direction, milestone branches base on and merge into `main` directly (no integration branch).
